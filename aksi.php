...
elseif($mod=='dosen_tambah'){
    ...
    $agama              = $_POST['agama'];
    $alamat             = $_POST['alamat'];
    $prodi_id           = get_prodi_registrasi();
    $nama_bidangilmu         = 0;
    if(!$kode_dosen || !$nama_dosen || !$nidn || !$nodos || !$jenis_kelamin || !$gambar['tmp_name'] || !$pendidikan_terakhir || !$tempat_lahir || !$tanggal_lahir || !$agama || !$alamat || !$prodi_id)
        print_msg("Field bertanda * tidak boleh kosong!");
    elseif(is_numeric($nidn)==false || is_numeric($nodos)==false)
    ...
    if($kode_dosen=='' || $nama_dosen=='' || $nidn=='' || $nodos=='' || $jenis_kelamin=='' || $pendidikan_terakhir=='' || $tempat_lahir=='' || $tanggal_lahir=='' || $agama=='' || $alamat=='')
        print_msg("Field bertanda * tidak boleh kosong!");
    elseif(is_numeric($nidn)==false || is_numeric($nodos)==false)
        ...
        $db->query("UPDATE tb_dosen SET kode_dosen='$kode_dosen', nama_dosen='$nama_dosen', nidn='$nidn', nodos='$nodos', jenis_kelamin='$jenis_kelamin', $sql_gambar, pendidikan_terakhir='$pendidikan_terakhir', tempat_lahir='$tempat_lahir', tanggal_lahir='$tanggal_lahir', agama='$agama', alamat='$alamat' WHERE id_dosen='$_GET[ID]'"); 
        }
