...
$rows = $db->get_results("
    SELECT d.id_dosen, d.nidn, d.nama_dosen, d.hitung, d.prodi_id, k.nama_kriteria AS hasil
    FROM tb_dosen d
    LEFT JOIN tb_penelitian p ON d.id_dosen = p.nidn
...
function get_rules_option($selected = '')
{
    global $db;
    $rows = $db->get_results("SELECT id_kriteria, nama_kriteria FROM tb_kriteria ");
    foreach ($rows as $row) {
        var_dump($selected);
        if ($row->nama_kriteria == $selected)
            $a .= "<option value='$row->nama_kriteria' selected>$row->nama_kriteria</option>";
        else
            $a .= "<option value='$row->nama_kriteria'>$row->nama_kriteria</option>";
    }
    return $a;
}
...
function get_kriteria_name_option($selected = '')
{
    var_dump($selected);
    global $db;
    $rows = $db->get_results("SELECT id_kriteria, nama_kriteria FROM tb_kriteria ORDER BY id_kriteria DESC");
